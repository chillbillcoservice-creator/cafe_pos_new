...
export default function KitchenOrders({ orders, setOrders }: KitchenOrdersProps) {
  ...
  const moveAllOrders = (fromStatus: OrderStatus, toStatus: OrderStatus) => {
    setOrders(orders.map(order => order.status === fromStatus ? { ...order, status: toStatus } : order));
  };
  ...
  const getOrderIdForDisplay = (orderId: string) => {
    const numericId = parseInt(orderId, 10);
    if (!isNaN(numericId)) {
      return `#${String(numericId).padStart(3, '0')}`;
    }
    return `#${orderId}`; // Fallback for non-numeric IDs
  };
