...
export default function BillHistory({ bills, onClearAll, currency = 'Rs.' }: BillHistoryProps) {
  ...
  const filteredBills = useMemo(() => {
    ...
    if (date) {
      return bills.filter(bill => isSameDay(new Date(bill.timestamp), date));
    }

    switch (filter) {
      case 'month':
        return bills.filter(bill => isSameMonth(new Date(bill.timestamp), now) && isSameYear(new Date(bill.timestamp), now));
      case 'year':
        return bills.filter(bill => isSameYear(new Date(bill.timestamp), now));
      case 'all':
      default:
        return bills;
    }
  ...
  const handleDeleteBill = async (billId: string) => {
    try {
      ...
      // This will require a way to update the parent state
    } catch (error) {
            ...
            {filteredBills.length > 0 ? (
              filteredBills.map((bill) => (
                <TableRow key={bill.id}>
                  ...
                  <TableCell>
                    {format(new Date(bill.timestamp), 'PPP p')}
                  </TableCell>
      ...
      <Dialog open={!!selectedBill} onOpenChange={() => setSelectedBill(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            {selectedBill && (
              <DialogDescription>
                {getBillTitle(selectedBill)} | Date: {format(new Date(selectedBill.timestamp), 'PPP p')}
              </DialogDescription>
            )}
          </DialogHeader>
