...
const processData = (bills: Bill[], type: ReportType) => {
  ...
  const safeBills = bills.filter(b => b.timestamp);

  switch (type) {
    case 'daily':
      filteredBills = safeBills.filter(bill => isSameDay(new Date(bill.timestamp), now));
      ...
      break;
    case 'monthly':
      filteredBills = safeBills.filter(bill => isSameMonth(new Date(bill.timestamp), now) && isSameYear(new Date(bill.timestamp), now));
      ...
      filteredBills.forEach(bill => {
        const day = new Date(bill.timestamp).getDate() - 1;
        days[day].revenue += bill.total;
      });
      data = days;
      break;
    case 'yearly':
      filteredBills = safeBills.filter(bill => isSameYear(new Date(bill.timestamp), now));
      ...
      filteredBills.forEach(bill => {
        const month = getMonth(new Date(bill.timestamp));
